...
try {
    ...
    $sql = "SELECT id, name, quantity FROM category";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);    // if result is present
    if (count($result) > 0) {
        echo "<a href='/shop/category/add.php' style='text-decoration:none;'>
                      <button>Add</button>
                  </a>";
        ...
        foreach ($result as $row) {
            ...
            echo "<td>
                  <a href='/shop/category/edit.php?id=" . htmlspecialchars($row['id']) . "' style='text-decoration:none;'>
                      <button>Edit</button>
                  </a>
                  <a href='/shop/category/delete.php?id=" . htmlspecialchars($row['id']) . "' style='text-decoration:none;'>
                      <button>Delete</button>
                  </a>
                  <a href='/shop/category/view.php?id=" . htmlspecialchars($row['id']) . "' style='text-decoration:none;'>
                      <button>View</button>
                  </a>
              </td>";
            echo "</tr>";
